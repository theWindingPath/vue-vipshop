<template>
  <transition name="slide">
    <div class="shop-center">
      <div class="header">
        <div class="back" @click="back">
          <i class="icon-fanhui"></i>
        </div>
        <h1 class="title">购物车</h1>
      </div>
      <shop-list :productList="productList"></shop-list>
      <!-- <div class="middle">
        <scroll ref="scroll" :data="shopList" class="scroll-list">
          <div class="products-wrapper">
            <ul class="cart-order-list">
              <li v-for="product in shopList" :key="product.id" class="list-item">
                <div class="icon">
                  <img width="76px" height="96px" v-lazy="product.small_image">
                </div>
                <div class="content">
                  <div class="name">
                    <div class="product-name">唯品自营 | {{product.product_name}}</div>
                    <div class="brand-name">{{product.brand_show_name}}特卖旗舰店</div>
                  </div>
                  <div class="price">
                    <p class="new-price">￥{{product.vipshop_price}}</p>
                    <del class="old-price">￥{{product.market_price}}</del>
                  </div>
                  <div class="cart-control clearfix">
                    <div class="cart-decrease number-bottom">﹣</div>
                    <div class="cart-count">{{product.count}}</div>
                    <div class="cart-add number-bottom">+</div>
                    <div class="cart-delete">×</div>
                  </div>
                </div>
              </li>
            </ul>
          </div>
        </scroll>
        <div class="shopping-cart-empty" v-show="totalPrice===0">
          <div class="bgImg"></div>
          <h2 class="subtitle">购物车空空如也</h2>
        </div>
      </div> -->
      <div class="bottom" v-show="totalPrice>0">
        <div class="price">总金额<span class="total-price">￥{{totalPrice}}</span></div>
        <button class="paycheck">结算</button>
      </div>
    </div>
  </transition>
</template>

<script type="text/ecmascript-6">
import { mapGetters } from 'vuex'
// import Scroll from 'base/scroll/scroll'
import ShopList from 'components/shop-center-list/shop-center-list'

export default {
  data() {
    return {
      productList: []
    }
  },
  created() {
    this.productList = this.shopList
    console.log(this.productList)
    console.log(this.shopList)
  },
  methods: {
    back() {
      this.$router.back()
    }
  },
  computed: {
    totalPrice() {
      let total = 0
      this.shopList.forEach((product) => {
        total += (product.vipshop_price * product.count)
      })
      return total
    },
    ...mapGetters([
      'shopList'
    ])
  },
  components: {
    ShopList
  },
  watch: {
      $route(newVal, oldVal) { // 路由变化、跳转
        // console.log(newVal, oldVal)
        if (newVal.fullPath === '/shop') { // 路由到购物中心组件，才更新数据s
          this.productList = this.shopList
          console.log('update productList')
        }
        // if (newVal !== oldVal) { // 路由变化，更新数据
        //   this.productList = this.shopList
        //   console.log('update productList')
        // }
      }
  }
}
</script>

<style scoped lang="stylus" rel="stylesheet/stylus">
  @import "~common/stylus/variable"

  .shop-center
      position fixed
      width 100%
      height 100%
      left 0
      right 0
      top 0
      bottom 0
      z-index 150
      background #fff
      color $color-theme
      &.slide-enter-active, .slide-leave-active
          transition all 0.3s
          .header, .bottom
            transition all 0.4s cubic-bezier(0.86, 0.18, 0.82, 1.32)
      &.slide-enter, .slide-leave-to
          transform translate3d(100%, 0, 0) // 从右边进入 缓动动画s
          .header
            transform translate3d(0, -100px, 0)
          .bottom
            transform translate3d(0, 100px, 0)
      .header
          position relative
          z-index 40
          width 100%
          height 40px
          background-color #fff
          border-bottom 1px solid rgba(7, 17, 27, 0.1)
          .back
              position absolute
              top 0
              // left 6px
              left 3px
              z-index 50
              .icon-fanhui
                  display block
                  // padding 10px // 增加点击面积
                  // font-size $font-size-large-x
                  padding 2px 6px
                  font-size 32px
                  // color $color-theme
                  color #222
                  font-weight 400
          .title
              position absolute
              top 0
              left 10%
              z-index 40
              width 80%
              text-align center
              height 40px
              line-height 40px
              font-size $font-size-large
              color $color-theme
              font-weight 700
      // .middle
      //   position fixed
      //   width 100%
      //   top 40px
      //   bottom 80px
      //   .scroll-list
      //     height 100%
      //     overflow hidden
      //     .products-wrapper
      //       width 100%
      //       .cart-order-list
      //         margin 6px 16px
      //         .list-item
      //           display flex
      //           width 100%
      //           padding 16px 0
      //           font-size 14px
      //           .icon
      //             flex 0 0 76px
      //             margin-right 12px
      //           .content
      //             flex 1
      //             position relative
      //             .name
      //               color #555
      //               .product-name
      //                 display -webkit-box
      //                 margin-bottom 6px
      //                 padding-right 52px
      //                 line-height 18px
      //                 color #222
      //                 overflow hidden
      //                 text-overflow ellipsis
      //                 -webkit-box-orient vertical
      //                 -webkit-line-clamp: 2
      //               .brand-name
      //                 line-height 12px
      //                 color #999
      //                 font-size 12px
      //             .price
      //               position absolute
      //               right 0
      //               top 0
      //               width 88px
      //               text-align right
      //               .new-price
      //                 margin-bottom 6px
      //                 line-height 16px
      //                 font-size 16px
      //                 color #222
      //                 font-weight 500
      //               .old-price
      //                 line-height 12px
      //                 font-size 12px
      //                 color #98989f
      //             .cart-control
      //               position relative
      //               top 22px
      //               height 26px
      //               &.clearfix
      //                 zoom 1
      //                 clear both
      //               .number-bottom
      //                 float left
      //                 padding 0 2px
      //                 width 20px
      //                 height 20px
      //                 line-height 20px
      //                 text-align center
      //                 border 1px solid #e7e7e7
      //                 color #83878f
      //                 font-size 18px
      //               .cart-decrease
      //                 border-radius 4px 0 0 4px
      //               .cart-add
      //                 border-radius 0 4px 4px 0
      //               .cart-count
      //                 float left
      //                 height 20px
      //                 line-height 20px
      //                 padding 0 10px
      //                 border-top 1px solid #e7e7e7
      //                 border-bottom 1px solid #e7e7e7
      //                 color #595d65
      //               .cart-delete
      //                 position absolute
      //                 right 0
      //                 top 50%
      //                 transform translateY(-50%)
      //                 color #a2a4a8
      //                 font-size 22px
        // .shopping-cart-empty
        //   position relative
        //   padding 60px 0
        //   background-color #fff
        //   text-align center
        //   .bgImg
        //     width 120px
        //     height 160px
        //     background-image url('data:image/png;base64,')
        //     background-size 120px 160px
        //     margin 0 auto
        //   .subtitle
        //     margin-top 22px
        //     color $color-text-l
        //     font-size 16px
        //     font-weight 400
      .bottom
        position fixed
        bottom 0
        left 0
        width 100%
        height 80px
        z-index 40
        background-color #fff
        border-top 1px solid rgba(7, 17, 27, 0.1)
        .price
          width 100%
          height 30px
          line-height 30px
          text-align center
          z-index 40
          color $color-text-ll
          font-size 14px
          font-weight 600
          .total-price
            margin-left 3px
            color #e4007f
            font-size 16px
            font-weight 700
        .paycheck
          position absolute
          left 10%
          width 80%
          height 40px
          border none
          background-color #e80080
          color #fff
          font-size 16px

</style>